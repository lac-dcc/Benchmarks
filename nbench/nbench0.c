...
#include "sysinfo.c"
  ...
  */
  if (global_custrun == 0) {
    output_string("==========================ORIGINAL BYTEMARK "
                  "RESULTS==========================\n");
    ...
    output_string(buffer);
    output_string("Baseline (MSDOS*)   : Pentium* 90, 256 KB L2-cache, Watcom* "
                  "compiler 10.0\n");
#ifdef LINUX
    output_string("==============================LINUX DATA "
                  "BELOW===============================\n");
...
#include "sysinfoc.c"
    ...
    output_string(buffer);
    output_string("Baseline (LINUX)    : AMD K6/233*, 512 KB L2-cache, gcc "
                  "2.7.2.3, libc-5.4.38\n");
#endif
                           ...
                           double *sdev)            /* Sample stand dev */
{
  ...
  if ((num_scores < 2) || (num_scores > 30)) {
    output_string("Internal error: calc_confidence called with an illegal "
                  "number of scores\n");
    return (-1);
  }
  /*
