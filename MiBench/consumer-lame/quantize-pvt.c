...
longer used) 4:  convert_mdct = 1, convert_psy=0,  reduce_sidechannel=1
...
void quantize_xrpow_ISO(FLOAT8 xr[576], int ix[576], gr_info *cod_info) {
...
#endif
  {
    ...
    /* depending on architecture, it may be worth calculating a few more
       compareval's. eg.  compareval1 = (2.0 - 0.4054/istep);
       ...
       the second compare becomes more expensive than just calculating the
       value. Architectures with slow FP operations may want to add some more
       comparevals. try it and send your diffs statistically speaking 73% of all
       xr*istep values give ix=0 16% will give 1 4%  will give 2
    */
    for (j = 576; j > 0; j--) {
      if (compareval0 > *xr) {
        *(ix++) = 0;
