...
void lame_help(lame_global_flags *gfp, char *name) /* print syntax & exit */
{
  ...
  fprintf(stdout, "    -s sfreq        sampling frequency of input file(kHz) - "
                  "default 44.1kHz\n");
  ...
  fprintf(stdout, "  --lowpass freq         frequency(kHz), lowpass filter "
                  "cutoff above freq\n");
  fprintf(stdout, "  --lowpass-width freq   frequency(kHz) - default 15%% of "
                  "lowpass freq\n");
  fprintf(stdout, "  --highpass freq        frequency(kHz), highpass filter "
                  "cutoff below freq\n");
  fprintf(stdout, "  --highpass-width freq  frequency(kHz) - default 15%% of "
                  "highpass freq\n");
  fprintf(stdout, "  --resample sfreq  sampling frequency of output file(kHz)- "
                  "default=input sfreq\n");
  fprintf(stdout, "  --cwlimit freq    compute tonality up to freq (in kHz) "
                  "default 8.8717\n");
  fprintf(stdout, "\n");
  fprintf(stdout, "  Operational options:\n");
  fprintf(stdout, "    -m mode         (s)tereo, (j)oint, (f)orce or (m)ono  "
                  "(default j)\n");
  fprintf(
      stdout,
      "                    force = force ms_stereo on all frames. Faster\n");
  fprintf(stdout, "    -a              downmix from stereo to mono file for "
                  "mono encoding\n");
  fprintf(stdout,
  ...
  fprintf(stdout, "    --preset type   type must be phone, voice, fm, tape, "
                  "hifi, cd or studio\n");
  ...
  fprintf(stdout, "    -p              error protection.  adds 16bit checksum "
                  "to every frame\n");
  ...
  fprintf(stdout, "     --ty \"year\"      year in which the song/album was "
                  "made (max 4 chars)\n");
  fprintf(stdout, "     --tc \"comment\"   additional info (max 30 chars)\n");
  fprintf(stdout, "                      (or max 28 chars if using the "
                  "\"track\" option)\n");
  fprintf(
      stdout,
      ...
      if (*token == '-') {
        ...
        /* options for ID3 tag */
          ...
          if (gfp->lowpassfreq < 1) {
            fprintf(stderr, "Must specify lowpass with --lowpass freq, freq >= "
                            "0.001 kHz\n");
            exit(1);
          }
          ...
          if (gfp->lowpasswidth < 0) {
            fprintf(stderr, "Must specify lowpass width with --lowpass-width "
                            "freq, freq >= 0 kHz\n");
            exit(1);
          }
          ...
          if (gfp->highpassfreq < 1) {
            fprintf(stderr, "Must specify highpass with --highpass freq, freq "
                            ">= 0.001 kHz\n");
            exit(1);
          }
          ...
          if (gfp->highpasswidth < 0) {
            fprintf(stderr, "Must specify highpass width with --highpass-width "
                            "freq, freq >= 0 kHz\n");
            exit(1);
          }
           ...
           * exit) quiet/silent  => no messages on screen brief         => few
           * messages on screen (name, status report) verbose       => all infos
           * to screen (brhist, internal flags/filters) o/output file =>
           * specifies output filename O             => stdout i/input file  =>
           * specifies input filename I             => stdin
           */
...
#ifdef LIBSNDFILE
            fprintf(stderr, "WARNING: libsndfile may ignore -r and perform "
                            "fseek's on the input.\n");
            fprintf(stderr,
