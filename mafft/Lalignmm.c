...
//	short **shortmtx;
  ...
  if (lgth1 < DPTANNI || lgth2 < DPTANNI) // zettai ni anzen ka?
  {
    ...
    //		value = MSalignmm_tanni( icyc, jcyc, eff1, eff2, seq1, seq2, cpmx1,
    //cpmx2, ist, ien, jst, jen, alloclen, aseq1, aseq2, gapinfo );
...
//			g = *mjpt + fpenalty;
      ...
      *curpt += wm;
      ...
      if (i == imid) // muda
      {
        ...
        //is %d\n", j, *mpjpt ); 				fprintf( stderr, "jumpbacki[%d] in forward dp
        //is %d\n", j, mpi );
        midw[j] = *curpt;
        midm[j] = *mjpt;
        midn[j] = mi;
  ...
  for (i = lgth1 - 2; i > -1; i--) {
    ...
    for (j = lgth2 - 2; j > -1; j--) {
      ...
      if (i == jumpi || i == imid - 1) {
        ...
        //ijpi ); 				fprintf( stderr, "jumpforj[%d] = %d\n", j, ijpj );
      }
      if (i == imid) // muda
      {
        midw[j] += wm;
...
//);
    ...
    if (i == jumpi) // saki?
    {
      ...
      //jumpi ); 			fprintf( stderr, "jmid, jumpj = %d,%d\n", jmid, jumpj );
      if (jmid == 0) {
...
//          if( WMMTX[i][j] > amino_dis['a']['g'] -1 )
	...
	fprintf( stderr, "\n" );
      ...
      //	Atracking( currentw, lastverticalw, seq1, seq2, mseq1, mseq2,
      //cpmx1, cpmx2, ijp, icyc, jcyc );
...
//          if( WMMTX[i][j] > amino_dis['a']['g'] -1 )
  ...
  //aseq1[0], ist, ien ); 	fprintf( stderr, "====(f) aseq2[0] (%d) = %s
  ...
  if (l > 0) {
    ...
    //ogcp1[i] ); 		for( i=0; i<lgth1; i++ ) fprintf( stderr, "fgcp1[%d] = %f\n",
    //i, fgcp1[i] );
  ...
  //aseq1[0], ist, ien ); 	fprintf( stderr, "====(s) aseq2[0] (%d) = %s
  //(%d-%d)\n", depth, aseq2[0], jst, jen );
