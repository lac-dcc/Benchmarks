...
static void pairalign(char name[M][B], int nlen[M], char **seq, char **aseq,
                      char **mseq1, char **mseq2, double *effarr,
                      int alloclen) {
  int i, j, ilim;
  int clus1, clus2;
  int off1, off2;
  ...
  if (alg == 's') {
    ...
    preparebpp(njob, bpp);
    fprintf(stderr, "done.\n");
    fprintf(stderr, "Running MXSCARNA (Tabei et al. "
  ...
  for (i = 0; i < ilim; i++) {
    ...
    for (j = i + 1; j < njob; j++) {
      ...
      //		fprintf( stderr, "mseq2 = %s\n", mseq2[0] );
			...
			fprintf( stderr, "\n" );
#endif
      //		for( l=0; l<clus1; l++ ) fprintf( stderr, "## STEP-eff
      //for mseq1-%d %f\n", l, effarr1[l] );
...
#endif
      {
        ...
        case ('L'):
          ...
          //); 						pscore = (float)naivepairscore11( *mseq1, *mseq2, penalty ); //
          //nennnotame 						fprintf( stderr, "pscore (2) = %f\n\n", pscore );
          break;
        case ('H'):
          pscore =
        ...
        case ('s'):
          ...
          //						pscore = G__align11_noalign( amino_disLN,
          //-1200, -60, mseq1, mseq2, alloclen );
          off1 = off2 = 0;
          //						fprintf( stderr, "scarna, pscore = %f\n",
          //pscore );
          break;
        case ('M'):
          //						pscore = MSalign11( mseq1, mseq2, effarr1,
          //effarr2, clus1, clus2, alloclen, NULL, NULL, NULL, NULL );
      ...
      if (alg == 't' ||
          (mseq1[0][0] != 0 &&
           mseq2[0][0] != 0)) // 't' no jouken ha iranai to omou. if( (
                              // mseq1[0][0] != 0 && mseq2[0][0] != 0  ) )
      {
...
//tmpptr->start2, tmpptr->end2, (void *)tmpptr->next ); 					else
#ifdef LLVM
