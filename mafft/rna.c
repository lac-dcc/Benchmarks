  ...
  for (i = 0; i < nseq; i++) {
    ...
    for (j = 0; j < nogaplgth; j++)
      for (pt = single[i][j]; pt->bestpos != -1; pt++) {
        ...
        //prob ); 			fprintf( stderr, "pairprob[0][0].bestpos=%d\n",
        //pairprob[0][0].bestpos ); 			fprintf( stderr,
        //"pairprob[0][0].bestscore=%f\n", pairprob[0][0].bestscore );
      }
  }
...
#else
    if (prob > 0.0) {
      pairprob[left] = (RNApair *)realloc(pairprob[left], (pairnum[left] + 2) *
                                                              sizeof(RNApair));
  ...
  if (1) {
            ...
            for (pairpt2 = pairprob2[j]; pairpt2->bestpos != -1; pairpt2++) {
              ...
              if (uido > -1 && ujdo > -1) {
                if (uido > i && j > ujdo) {
                  impmtx2[i][j] +=
                      prob *
                      pairedribosumscore53(nseq1, nseq2, oseq1, oseq2, eff1,
                                           eff2, i, j, uido, ujdo) *
                      consweight_multi;
                  tbppmtx[i][j] -= prob;
                  ...
                  impmtx2[i][j] +=
                      prob *
                      pairedribosumscore35(nseq1, nseq2, oseq1, oseq2, eff1,
                                           eff2, i, j, uido, ujdo) *
                      consweight_multi;
                  tbppmtx[i][j] -= prob;
                }
      ...
      for (pairpt1 = pairprob1[i]; pairpt1->bestpos != -1; pairpt1++) {
        ...
        for (j = 0; j < lgth2; j++)
          for (pairpt2 = pairprob2[j]; pairpt2->bestpos != -1; pairpt2++) {
            ...
            //pairpt1-pairprob1[i], pairpt2-pairprob2[j] ); 				if( pairprob2[j] ==
            //NULL ) continue;
    ...
    for (i = 0; i < lgth1; i++)
      for (j = 0; j < lgth2; j++) {
        ...
        //consweight_multi, consweight_rna ); 			impmtx[i][j] *= 0.5;
      }
