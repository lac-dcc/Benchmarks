...
void intergroup_score_consweight(char **seq1, char **seq2, double *eff1,
  ...
  *value = 0.0;
  for (i = 0; i < clus1; i++) {
    for (j = 0; j < clus2; j++) {
      efficient = eff1[i] * eff2[j]; /* なぜか配列を使わないとおかしくなる,
                                        多分バグ */
      mseq1 = seq1[i];
      mseq2 = seq2[j];
      tmpscore = 0.0;
...
void fixed_musclesupg_float_realloc_nobk_halfmtx_treeout(
    int nseq, float **eff, int ***topol, float **len, char **name, int *nlen) {
  int i, j, k, miniim, maxiim, minijm, maxijm;
  int *intpt, *intpt2;
  float tmpfloat;
  float eff1, eff0;
...
void dontcalcimportance(int nseq, double *eff, char **seq,
                        LocalHom **localhom) {
  int i, j;
  LocalHom *ptr;
...
static void addlocalhom2_e(LocalHom *pt, LocalHom *lh, int sti, int stj,
                           int eni, int enj, double opt, int overlp,
                           int interm) {
  // dokka machigatteru
  if (pt != lh) // susumeru
  {
      ...
      for (pk = 0; pk < plim; pk++) {
        ...
        if (status) {
              ...
              inj[pk - 1] != inj[pk] - 1) // saigonoshori
          {
            ...
            //%p\n", pt, pt->next ); 							fprintf( stderr, "in ex (ba), pt = %p,
            //pt->next = %p, k=%d\n", pt, pt->next, k );
            addlocalhom2_e(
...
void getdiaminofreq_st(float *freq, int clus, char **seq, double *eff,
                       int len) {
  int i, j;
  float feff;
  for (i = 0; i < len + 1; i++)
...
void getdiaminofreq_part(float *freq, int clus, char **seq, double *eff,
  ...
  for (i = 0; i < clus; i++) {
    ...
    //nomi
    if (egappat[i] != '-' &&
        seq[i][len - 1] != '-') // xxx wo tsukawanaitoki [len-1] nomi
      freq[len] += feff;
  }
  //	fprintf( stderr, "\ndiaaf = \n" );
  //	for( i=0; i<len+1; i++ ) fprintf( stderr, "%5.3f ", freq[i] );
...
void st_getGapPattern(Gappat **pat, int clus, char **seq, double *eff,
                      int len) {
  int i, j, k, gb, gc;
  int known;
  float feff;
  ...
  for (i = 0; i < n1; i++)
    for (j = 0; j < n2; j++) {
      ...
      while (*p1p) {
        ...
        //amino_dis[*p1][*p2] ); 			if( pv ) fprintf( stderr, "Penal!, %f, %d-%d,
        //pos1,pos2=%d,%d\n", pv * deff * 0.5,  i, j, p1-seq1[i], p2-seq2[j] );
