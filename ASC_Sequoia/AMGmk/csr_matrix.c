 ...
 *B. Data is not computed, only structural information is created. A and B must
 *have the same numbers of rows. Nothing is done about Rownnz.
 *
...
hypre_CSRMatrix *hypre_CSRMatrixUnion(hypre_CSRMatrix *A, hypre_CSRMatrix *B,
  ...
  if (col_map_offd_A ==
      0) { /* The matrices are diagonal blocks.
  ...
  /* ==== The first run through A and B is to count the number of nonzero
     elements, without double-counting duplicates.  Then we can create C. */
