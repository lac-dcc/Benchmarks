...
int gz_fill_rectangle(int x, int y, int w, int h, gx_device_color *pdevc,
  ...
  if (w <= tile->width && h <= tile->height) {
    ...
    if (xmod + w <= tile->width &&
        (ymod = y % tile->height) + h <= tile->height) { /* Just do a copy. */
  ...
  if (code < 0) { /* Use the default implementation */
...
int gz_fill_trapezoid_fixed(
    fixed fx0, fixed fw0, fixed fy0, fixed fx1, fixed fw1, fixed fh,
    int swap_axes, gx_device_color *pdevc,
    gs_state *pgs) { /* For the moment, we just convert everything to ints. */
...
int gx_default_tile_rectangle(
    gx_device *dev, register gx_bitmap *tile, int x, int y, int w, int h,
    gx_color_index color0,
    gx_color_index color1) { /* Fill the rectangle in chunks */
