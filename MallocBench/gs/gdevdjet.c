...
int compress_row(char *row, char *end_row, char *compressed) {
  ...
  while (i_exam < end_row) { /* Search ahead in the input looking for a run */
    ...
    while (i_compr < i_exam) { /* Encode up to 127 dissimilar bytes */
    ...
    while (i_exam < i_next) { /* Encode up to 127 similar bytes */
