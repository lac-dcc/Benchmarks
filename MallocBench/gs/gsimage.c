...
int image_init(register gs_image_enum *penum, int width, int height,
  ...
  if (spp == 1) { /* Initialize the color table */
#define chtl(i) penum->dev_colors[i].halftone_level
    switch (log2_bps) {
    case 3: { /* Yes, clearing the entire table is slow, */
...
int gs_image_next(register gs_image_enum *penum, byte *dbytes, uint dsize) {
  ...
  while (dleft) { /* Fill up a row, then display it. */
...
int image_render_direct(gs_image_enum *penum, byte *data, uint w, int h) {
  ...
  if (ht == 1) { /* We can do the whole thing at once. */
    (*proc)(dev, data, 0, (w + 7) >> 3, ix, iy, w, h, zero, one);
    return h;
  } else { /* Do just one row. */
    int dy;
    if (ht < 0)
      iy += ht, ht = -ht;
    for (dy = 0; dy < ht; dy++)
...
#define image_set_gray(sample_value)                                           \
  ...
  if (!skew) { /* We're going to round to pixel boundaries later, */
  ...
  /* to handle final run */
  {
    if (*psrc++ != run) { /* Fill the region between */
      /* xrun and xl */
      if (penum->slow_loop) { /* Must use general fill */
        gx_path ipath;
        gx_path *ppath = &ipath;
        int code;
        ...
        if (!skew) { /* Do the empty/clipping check */
...
#undef check_clipped_or_empty
      ...
      nofill:
        gx_path_release(ppath);
      trans:;
      } else { /* No clipping, no skew, and not */
