...
int gx_path_bbox(gx_path *ppath, gs_fixed_rect *pbox) {
  if (ppath->first_subpath ==
      0) { /* The path is empty, use the current point if any. */
  ...
  if (ppath->subpath_count == 1 && ppath->segment_count == 4 &&
    ...
    if ((x0 == pseg1->pt.x && pseg1->pt.y == y2 && x2 == pseg3->pt.x &&
        ...
        (x0 == pseg3->pt.x && pseg3->pt.y == y2 && x2 == pseg1->pt.x &&
         pseg1->pt.y == y0)) { /* Path is a rectangle.  Return bounding box. */
      if (x0 < x2)
...
int gx_path_merge(
    gx_path *ppfrom,
    gx_path *ppto) { /* If no new segments, don't release the parent. */
...
/* Flatten a path */
int gx_path_flatten(gx_path *ppath_old, gx_path *ppath,
                    floatp flatness) { /* See the flattening algorithm below for
                                          an explanation of */
  ...
  { /* Compute the maximum distance of the curve from */
    ...
    if ((dx3 < 0 ? -dx3 : dx3) >= (dy3 < 0 ? -dy3 : dy3)) {
      ...
      if (((dist = (fixed)(t * (x1 - x0)) - y1 + y0) < 0 ? -dist : dist) <= d &&
          ((dist = (fixed)(t * (x2 - x0)) - y2 + y0) < 0 ? -dist : dist) <=
              d) { /* Curve is flat enough.  Add a line and exit. */
#ifdef DEBUG
        if (gs_debug['u'])
      ...
      if (((dist = (fixed)(t * (y1 - y0)) - x1 + x0) < 0 ? -dist : dist) <= d &&
          ((dist = (fixed)(t * (y2 - y0)) - x2 + x0) < 0 ? -dist : dist) <=
              d) { /* Curve is flat enough.  Add a line and exit. */
#ifdef DEBUG
        if (gs_debug['u'])
          print_curve_point(x3, y3);
