...
#define accum_x(dx)                                                            \
  ...
  if (fc.skewed)                                                               \
  pty += c_fixed(dx, fc.xy)
#define accum_y(dy)                                                            \
  pty += c_fixed(dy, fc.yy);                                                   \
  if (fc.skewed)                                                               \
  ptx += c_fixed(dy, fc.yx)
#define accum_xy(dx, dy)                                                       \
  ptx += c_fixed(dx, fc.xx);                                                   \
  pty += c_fixed(dy, fc.yy);                                                   \
  if (fc.skewed)                                                               \
  pty += c_fixed(dx, fc.xy), ptx += c_fixed(dy, fc.yx)
...
top: /* Skip initial random bytes */
  ...
  while (1) {
    ...
    setp:
      ...
      goto pp;
    case c_endchar:
      dc("endchar") if (pis->seac_base >=
                        0) { /* We just finished the accent of a seac. */
      ...
      if (pis->charpath_flag) {
        ...
        if (gs_debug['1'])
          printf("[1]bbox=(%g,%g),(%g,%g)\n", bbox.p.x, bbox.p.y, bbox.q.x,
                 bbox.q.y);
#endif
        if (pis->paint_type) { /* Expand the bounding box to encompass */
        ...
        code = gs_setcachedevice(pis->penum, fixed2float(pis->width.x),
                                 fixed2float(pis->width.y), bbox.p.x, bbox.p.y,
                                 bbox.q.x, bbox.q.y);
        if (code < 0)
          return code;
