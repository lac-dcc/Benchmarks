...
int gx_fill_path(gx_path *ppath, gx_device_color *pdevc, gs_state *pgs,
  ...
  if (!(code = alloc_line_list(&lst, max_active))) {
    ...
    if (ibox.q.y <= cpath->cbox.q.y && ibox.q.x <= cpath->cbox.q.x &&
        ...
        ibox.p.x >= cpath->cbox.p
                        .x) { /* Path lies entirely within clipping rectangle */
      lst.no_clip = 1;
    } else {
      lst.no_clip = 0;
...
#define int_box(pqxy, rel)                                                     \
  ...
  ibox.pqxy = cpath->bbox.pqxy
      ...
      int_box(q.y, <);
#undef int_box
      if (ibox.p.x >= ibox.q.x ||
          ibox.p.y >= ibox.q.y) { /* Intersection of boxes is empty! */
        goto skip;
      }
  ...
  if (gs_debug['f'] || gs_debug['F']) {
    printf("[f]calls alloc   up   down  step slowx  iter  find  band bstep "
           "bfill\n");
...
void add_y_list(gx_path *ppath, short tag, line_list *ll, gs_fixed_rect *pbox) {
  ...
  while (pseg) {
    fixed dy;
    switch (pseg->type) { /* No curves left */
    case s_start:
      psub = (subpath *)pseg;
      plast = psub->last;
      dir = 0;
...
#define compute_dir(xo, xe, yo, ye)                                            \
  (xo > xmax && xe > xmax                                                      \
      ...
      if (pseg == plast) { /* The beginning of a subpath is always */
        /* treated like a horizontal line, */
        /* so the last segment must receive */
        /* special consideration. */
        if (pseg->type != s_line_close) { /* "Close" an open subpath */
          int cdir;
          py = psub->pt.y;
        ...
        } else { /* Just add the closing line if needed */
          if (dir < 0)
            add_y_line(prev, pseg, dir, ll);
        }
      }
...
void add_y_line(segment *prev_lp, segment *lp, int dir, line_list *ll) {
  ...
  prev.y = prev_lp->pt.y;
  alp->tag = ll->tag;
  if ((alp->direction = dir) > 0) { /* Upward line */
    y_start = prev.y;
...
void fill_loop(gx_device_color *pdevc, int rule, line_list *ll,
  ...
  while (1) {
    ...
    for (alp = alstop; stat(n_find_y), alp != 0;
         endp = alp, alp = alp->next) { /* Check for intersecting lines. */
      fixed nx = al_x_at_y(alp, y1);
      alp->x_next = nx;
      if (nx < x) { /* stop at intersection */
...
#define inside_path_p() ((inside[0] & rule) && (inside[1] & pgs->clip_rule))
      while (alp != 0) {
        ...
        alp->x_current = xtop;
        if (alp->end_mark >= end_count) { /* This line is ending here, or */
...
private
void swap_group(active_line *alp) { /* This line is the last of the group: */
...
void ended_line(register active_line *alp) {
  ...
  fixed y = pseg->pt.y;
  gs_fixed_point npt;
  if (alp->direction == dir_up) { /* Upward line, go forward along path */
    next = pseg->next;
    if (next == 0 || next->type == s_start)
      next = pseg; /* stop here */
  } else {         /* Downward line, go backward along path */
...
#endif
  if (npt.y <= y) { /* End of a line sequence */
    active_line *nlp = alp->next;
    alp->prev->next = nlp;
    if (nlp)
      nlp->prev = alp->prev;
