...
private
int stroke(gx_path *ppath,
           int (*line_proc)(P4(int, partial_line *, partial_line *,
                               gs_state *)),
           gs_state *pgs) {
  subpath *psub;
  int code;
...
#define next_line(ex, ey)                                                      \
  (pl.o.p.x = x, pl.o.p.y = y, pl.e.p.x = (ex), pl.e.p.y = (ey),               \
   ...
   0)
    ...
    psub = (subpath *)pline;
    if (stroke_path ==
        &stroke_path_body) { /* Fill and release the accumulated path */
...
int stroke_fill(int first, register partial_line *plp, partial_line *nplp,
  ...
  { /* General case. */
    ...
    if (stroke_path == 0) { /* We are rendering, and haven't run into the */
...
int stroke_add(int first, register partial_line *plp, partial_line *nplp,
  ...
  } else /* join_bevel or join_miter */
  {
    ...
    if (pgs->line_params->join ==
        gs_join_miter) { /* Check whether a miter join is appropriate. */
      ...
      if (u2 >= 0) {
        if (v2 >= 0) { /* 1st quadrant => negative tangent */
          if (num > 0)
            num = -num;
          if (denom < 0)
            denom = -denom;
        } else { /* 4th quadrant => use tan(b-a) */
          num = -num;
          if (denom < 0)
            num = -num, denom = -denom;
        }
      } else {
        if (v2 >= 0) { /* 2nd quadrant => use tan(a-b) */
          if (denom < 0)
            num = -num, denom = -denom;
        } else { /* 3rd quadrant => positive tangent */
          if (num < 0)
            num = -num;
          if (denom < 0)
            denom = -denom;
      ...
      if (num >= 0 ? (check > 0 && num >= denom * check)
                   : (check > 0 ||
                      num >= denom * check)) { /* OK to use a miter join. */
  ...
  case gs_cap_round: {
    ...
    if ((code = (*add_proc)(ppath, xo, yo)) < 0 ||
        (code = gx_path_add_arc(ppath, xo, yo, xm, ym, xo + cdx, yo + cdy)) <
            0 ||
        (code = gx_path_add_arc(ppath, xm, ym, xe, ye, xe + cdx, ye + cdy)) < 0)
      return code;
  } break;
  case gs_cap_square:
