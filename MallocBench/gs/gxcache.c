...
cached_char *gx_alloc_char_bits(gs_font_dir *dir, gx_device_memory *dev,
  ...
  if (dev->raster != 0 && iheight > dir->upper / dev->raster)
    return 0; /* too big */
  if (dir->csize >= dir->cmax ||
      dir->bmax - dir->bsize <
          isize) { /* There isn't enough room.  Clear the entire cache. */
...
void gx_add_cached_char(gs_font_dir *dir, cached_char *cc,
                        cached_fm_pair *pair) { /* Add the new character at the
                                                   tail of its chain. */
...
int gx_copy_cached_char(register gs_show_enum *penum,
                        register cached_char *cc) {
