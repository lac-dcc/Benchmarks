...
top:
  ...
  case '(':
    ...
    break;
  case '{':
    if (pstack == 0) { /* Use the operand stack to accumulate procedures. */
      myref = osp + 1;
  ...
  case '}':
    ...
    }
    {
      ...
      pstack = myref->size;
      if (pstack == 0 &&
          proc_da.is_dynamic) { /* Top-level procedure, shrink in place. */
        ...
        if (aref == 0)
          return e_VMerror;
        myref = pref;
      } else { /* Not top-level, or in ostack: copy it. */
      ...
      if (array_packing)
        make_tasv(myref, t_packedarray, a_executable + a_read + a_execute, size,
                  refs, aref);
      else
...
int scan_number(
    register stream *s,
    ref *pref) { /* Powers of 10 up to 6 can be represented accurately as */
...
#define num_powers_10 6
  ...
  while (isdigit(c)) { /* Check for overflowing ival */
...
fe: /* dval contains the value, negated if necessary */
  if (c == 'e' || c == 'E') { /* Check for a following exponent. */
    int esign = 0;
    long eexp;
    switch (c = sgetc(s)) {
    case '+':
