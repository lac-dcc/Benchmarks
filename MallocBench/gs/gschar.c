...
int gs_setcachedevice(register gs_show_enum *penum, floatp wx, floatp wy,
    ...
    return 0;
  {
    ...
    if (!penum->dev_cache_set) { /* Set up the memory device for the character
                                    cache */
...
int show_update(register gs_show_enum *penum) {
  ...
  case sws_cache: { /* Finish installing the cache entry. */
...
more:                     /* Proceed to next character */
  if (penum->can_cache) { /* Loop with cache */
    if (pair == 0)
    ...
    if ((index = penum->index++) == penum->size) { /* All done. */
      return 0;
    }
    chr = str[index];
  }
...
private
int stringwidth_move(
    register gs_show_enum *penum) { /* wxy is in device coordinates */
...
int stringwidth_proceed(register gs_show_enum *penum) {
  ...
  int code;
more: /* Proceed to next character */
  if ((index = penum->index++) ==
      penum->size) { /* All done.  Since this was a stringwidth, */
...
int show_setup(register gs_show_enum *penum, gs_state *pgs, char *str) {
  ...
  penum->index = 0;
  penum->continue_proc = continue_show;
  if (!pgs->char_tm_valid) {         /* Compute combined transformation */
