...
void zfile_init() { /* Create files for stdin, stdout, and stderr. */
  int i;
...
int zclosefile(register ref *op) {
  ...
  int code;
  check_file(s, op);
  if ((code = file_close(op, s)) >=
      0) { /* If we just closed the file from which the interpreter */
...
int zreadhexstring(register ref *op) {
  ...
  if (s == &st) { /* Reading from a string, return remainder */
    uint pos = stell(&st);
...
int zcurrentfile(register ref *op) {
  ref *fp;
  push(1);
  if ((fp = get_current_file()) == 0) { /* Return an invalid file object. */
...
int lib_file_open(byte *fname, uint len, ref *pfile) {
  ...
  for (ppath = gs_lib_paths; *ppath != 0; ppath++) {
    char *path = *ppath;
    for (;;) { /* Find the end of the next path */
      char *npath = path;
      uint plen;
      char *cstr;
      int clen;
...
int file_open(byte *fname, uint len, char *file_access, ref *pfile) {
  ...
  if (fname !=
      0) { /* Copy the name (so we can terminate it with a zero byte.) */
    ...
    if (file == 0 || (code = string_to_ref(file_name, &fe->file_name,
...
int open_std_file(ref *pfname, char *file_access, ref *pfile) {
  ...
  for (i = 0; i < num_std_files; i++)
    if (!bytes_compare(
            pfname->value.bytes, pfname->size, std_file_names[i],
            strlen(std_file_names[i]))) { /* This is a standard file */
      int attrs =
          (*file_access == 'r' ? a_read + a_execute : a_write + a_execute);
