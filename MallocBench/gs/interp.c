...
void interp_init(
    int ndict) { /* Initialize the guard entries on the operand stack */
...
sw: /* We have to populate enough cases of the switch statement */
  ...
  case plain_exec(t_operator): {
    ...
    iosp = osp;
    if (code != 0) { /* This might be a control operator that changed */
  ...
  case plain_exec(t_name): {
    ...
    case exec(t_packedarray):
      ...
      goto top;
    case plain_exec(t_operator): { /* Shortcut for operators. */
  ...
  case exec(
      t_file): { /* Executable file.  Read the next token and interpret it. */
    stream *s;
