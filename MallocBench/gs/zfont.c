...
int zdefinefont(register ref *op) {
  ...
  if (r_attrs(op) & a_write) { /* Assume this is a new font */
    gs_font *pfont;
    font_data *pdata;
    if (code > 0)
  ...
  } else { /* Assume this was made by makefont or scalefont */
    if (code <= 0)
...
private
int font_param(ref *fp,
               gs_font **pfont) { /* Check that fp is a read-only dictionary, */
...
int make_font(ref *op, gs_matrix *pmat) {
  ...
  *(gs_matrix *)(newmat.value.refs) = newfont->matrix;
  if (ffont) { /****** SHOULD DECREMENT REFCT ******/
  }
  *fp = newdict;
