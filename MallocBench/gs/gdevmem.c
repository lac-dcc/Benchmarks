...
int gs_device_is_memory(gx_device *dev) { /* We can't just compare the procs, or
                                             even an individual proc, */
...
int mem_copy_scan_lines(gx_device_memory *dev, int start_y, byte *str,
  ...
  while ((*dev->bring_in_proc)(dev, 0, y, bytes_per_line, count, 0) <
         0) { /* We can only split in Y, not in X. */
    uint part = count >> 1;
...
#define write_partial(msk)                                                     \
  if (fill)                                                                    \
  write_loop(*ptr |= msk) else write_loop(*ptr &= ~msk)
  switch (color) {
  case 0:
    fill = mdev->invert;
    break;
  ...
  if (bit + w <= chunk_bits) { /* Only one word */
...
private
int mem_mapped_map_color_rgb(gx_device *dev, gx_color_index color,
                             ushort *prgb) {
