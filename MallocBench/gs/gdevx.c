...
int x_open(register gx_device *dev) {
  ...
  if (geometry != NULL) { /*
                           ...
                           * use scr, because that is a Screen*, and XGeometry
                           * wants the screen number.
                           */
...
int x_tile_rectangle(register gx_device *dev, gx_bitmap *tile, int x, int y,
  ...
  if ((zero != xdev->ht.back_c) || (one != xdev->ht.fore_c))
    if (xdev->ht.bits)
      *xdev->ht.bits = ~*tile->data;

  set_back_color(zero);
  set_fore_color(one);
  set_function(GXcopy);
  if (!set_tile(dev,
                tile)) { /* Bad news.  Fall back to the default algorithm. */
    set_fill_style(FillSolid);
 ...
 */
{
  ...
  if (zero != gx_no_color_index) {
    if (one != gx_no_color_index) { /* Simply replace existing bits with what's
                                       in the image */
    } else if (bc == 0)
...
hard: /* Handle the hard 1-color case. */
  if (raster > xdev->cp.raster ||
      h > xdev->cp.height) { /* Must allocate a new pixmap and GC. */
...
out:
  if (xdev->bpixmap !=
      (Pixmap)0) { /* We wrote to the pixmap, so update the display now. */
    update_add(dev, x, y, w, h);
  }
