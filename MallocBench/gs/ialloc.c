...
char *alloc(uint num_elts, uint elt_size, char *client_name) {
  ...
  uint block_size;
  uint left;
  if (size >= ap->big_size) { /* Large object, do a separate malloc. */
    char *block = alloc_large(ap, size, client_name);
    if (block != NULL)
      return block;
  }
  block_size = align_round(size);
  if (block_size <= max_chain_size) { /* See if we can use a freed block. */
...
void alloc_free(char *cobj, uint num_elts, uint elt_size, char *client_name) {
  ...
  uint size = num_elts * elt_size;
  uint block_size;
  if (size >= ap->big_size) { /* Object was allocated with malloc. */
    alloc_free_large(cobj, size, client_name);
    return;
  }
#define obj ((byte *)cobj)
  else if (obj == ap->ctop) { /* Don't free the object if we're in a save and */
    ...
    return;
  } else if (obj + (block_size = align_round(size)) ==
             ap->cbot) { /* Freeing an aligned object.  Same check. */
    if (ap->save_level == 0 || ap->current.save_level >= ap->save_level ||
        obj >= ap->saved->state.cbot)
      ap->cbot = obj;
    alloc_print('-', '<', obj, size);
    return;
  } else if (!ptr_is_in_chunk(obj, &ap->current)) { /* In another segment, check
                                                       its save level. */
...
byte *alloc_grow(byte *obj, uint old_num, uint new_num, uint elt_size,
  ...
  if (new_size < ap->big_size) /* try to grow in place */
  {
    uint old_block_size;
    uint new_block_size;
    if (obj == ap->ctop) { /* Might be able to grow in place */
    ...
    if (obj + old_block_size == ap->cbot) { /* Might be able to grow in place */
...
byte *alloc_shrink(byte *obj, uint old_num, uint new_num, uint elt_size,
  ...
  if (new_size == old_size)
    return obj;
  if (old_size >= ap->big_size) { /* Allocate a new block. */
    ...
    alloc_print('<', '&', obj, new_size);
    return nobj;
  } else if (obj == ap->ctop) { /* Move the object up in place. */
...
int alloc_is_since_save(char *ptr, alloc_save *save) {
  ...
  if (ptr_is_in_chunk(ptr,
                      &save->state.current)) { /* In the chunk, check against
                                                  allocation pointers */
