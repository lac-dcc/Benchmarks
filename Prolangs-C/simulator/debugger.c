    ...
    case 'S':
      if (RESULT_TYPE == NUMBER_0)
        ...
        if (!(((**COMMAND) >= 'A') && ((**COMMAND) <= 'Z'))) {
          ...
          if (SYMB == NULL) {
            (void)printf("Unknown identifier, found '%s'. Ignoring command.\n",
                         SYMBOL);
            (*LEGAL) = FALSE_1;
            return 0;
          }
...
void HELP(char *COMMAND, BOOLEAN *LEGAL) {
  ...
  if ((*COMMAND) != '\0')
    ...
    (void)printf("   L<loc>     LOC<loc>        Examines contents of memory "
                 "and registers.\n");
    (void)printf("   L<loc>=<v> LOC<loc>=<v>    Examines contents of memory "
                 "and registers.\n");
