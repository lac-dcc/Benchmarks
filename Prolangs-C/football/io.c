...
void display_info(FILE *output, int team_code) {
  ...
  for (week = 1; week <= 21; week++) {
    ...
    case 19:
      (void)fprintf(
          output, "-(5/ ) Net points in %s %s  %d [%3.1f]", CONF_STR(team_code),
          DIVISION_STR(team_code), NET_POINTS(team_info_wi_div, team_code),
          PER_GAME(team_info_wi_div, NET_POINTS(team_info_wi_div, team_code),
                   team_code));
      break;
    case 20:
      (void)fprintf(
          output, "-( /4) Avg. net points in %s     %3.1f", CONF_STR(team_code),
          PER_GAME(team_info_wi_conf, NET_POINTS(team_info_wi_conf, team_code),
                   team_code));
      break;
    case 21:
      (void)fprintf(
...
void display_rankings(FILE *output) {
  ...
  (void)fprintf(
      output,
      "                   NFC                                       AFC\n");
  (void)fprintf(output, "                 REC.  OFF.  DEF.  NET                "
                        "    REC.  OFF.  DEF.  NET\n\n");
