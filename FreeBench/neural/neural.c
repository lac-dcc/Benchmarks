 ...
 * letters at once or step-by-step so that the progress can be studied closely.
 * Functions for unlearning using random vectors and for creating generators for
 * the stored patterns using the clamping method are also available. For the
 * function that find the closest stable state for a probe vector, both bipolar
 * and continuous versions are available. The program uses a simple menu system
 * for manouvering. The letters are read from the file 'w.txt'.
 *
...
generators[NUMPATS][NNTOT]; real Tmatrix[NNTOT][NNTOT];
*/
 ...
 * by the user until the user types 'q' to quit the program. */
int main(int c, char *v[]) {
  ...
  /* original menu system removed
   ...
   print original vectors"); printf("\nt - generate T-matrix\ns - generate
   T-matrix step-by-step"); printf("\nc - check if vectors are stored\nu -
   unlearning"); printf("\ng - compute generators for stored vectors and use the
   generators to print original vectors"); printf("\nm - print T-matrix\nz - set
   continuous neuron mode\nx - set two-valued neuron mode (default)");
      printf("\nq - quit program\nOption > ");
      option=getchar();
      getchar();
      switch(option) {
 ...
 * hamming distance of <2 is not allowed and the original vectors will have to
 * be changed. */
static void checkham() {
  int vec, comp, hd;
  int hamwarn = 0;
 ...
 * MODE_TRACE. MODE_ALL trains all 10 vectors at once and MODE_TRACE views the
 * current learning status between each vector. */
static void generateT(int mode) {
  int row, col, vec;
  char option = '0';
 ...
 * in dest. This function uses the sgn function for the updating. The number of
 * iterations needed to reach a stable state is returned. */
static int run(signed int *source, signed int *dest) {
 ...
 * written on the screen. If a vector is stored the word 'Stored' appears
 * beneath the typed pattern, otherwise the hamming distance between the
 * original and computed vector is displayed. The number of iterations needed to
 * reach a stable state is displayed below the "hamming distance or stored"
 * line. */
static void storecheck() {
  int vec, hd, *iter;
 ...
 * screen. -1's are printed as '.' and +1's as 'X' */
static void printvec(int vecs[][]) {
  int row, vec, neuron;
 ...
 * used to indicate the number of the vector so that the actual letter can be
 * printed from vnames. */
static void printV(int vector, signed int vect[]) {
  int neuron;
 ...
 * random seed and number of unlearning vectors (recommended about 100).
 ...
 * places the generators in the global variable 'generators'. The generators are
 * found using the clamping method. */
#ifndef BENCHMARK
static void makegenerators(int n) {
