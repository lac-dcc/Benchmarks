...
Maze2Mech(unsigned long bentNet,           /* net to bend */
          ...
          unsigned long xEnd, int bXdelta, /* direction of bend horz seg */
          unsigned long yStart,            /* rows to search across */
          unsigned long yEnd, int bYdelta) /* direction of bend vert seg */
{
  ...
  for (row = yStart; row != yEnd; row += bYdelta) { /* search for row */
    colFree = 1;
    for (col = xStart; colFree && (col != xEnd);
      ...
      for (topCol = topStart; topCol <= topEnd; topCol++) {
        for (botCol = botStart; botCol <= botEnd; botCol++) {
          if ((topCol != i) && (botCol != i) && (topRow != botRow) &&
              ...
              !HasVCV(botCol) &&
              ExtendOK(botNet, horzPlane, botCol, b2, i, b2)) {
            /* draw it! */
