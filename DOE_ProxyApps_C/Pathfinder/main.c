...
int main(int argc, char *argv[]) {
  ...
  if (!config) {
    printf("Error: Could not allocation configuration "
           "structure.\n\n\t---Exiting\n\n");
    exit(0);
  }
  ...
  if (argc < 2) {
    ...
    if ((cFlag + iFlag + xFlag) > 1) {
      fprintf(stderr, "-c -i and -x are mutually exclusive, PathFinder can "
                      "only do one of those.\n\n");
      return (1);
    }

    if ((cFlag + iFlag + xFlag) == 0) {
      fprintf(stderr, "-c (config file) -i (interactive) or -x (exhaustive) "
                      "must be specified.\n\n");
      return (1);
    }

    if (oFlag && !xFlag) {
      fprintf(stderr, "-o (optimized graph output) is only compatible with -x "
                      "(exhaustive searches).\n\n");
      return (1);
    }
...
void runInteractively(char *fileName, Configuration *config) {
  ...
  while (1) {
    ...
    do {
      ...
      printf("\nPlease insert a node label for this signature (\"\" to "
             "complete, \"bail\" to exit):\n");
      gets(stringBuffer);
