  ...
  for (edges = sourceNode->edges; edges != NULL; edges = edges->nextEdge) {
    if (outerNode) {
      ...
      if (node) {
         ...
         * sure the link here is appropriate.
         */
...
Graph *parseFile(char *fileName) {
  ...
  fileString = CharVector_new(1024); /* allocate our data parsing storage - 1024
                                        is a semi-arbitrary power of 2 */
  ...
  CharVector_getLineFromFile(fileString, inFile);
  if (!feof(inFile) &&
      (strncmp("Basic blocks:", fileString->string, 13) == 0)) {
  ...
  while (!feof(inFile)) /* clumsy use of feof */
  {
    ...
    if (strncmp("----------", fileString->string, 10) ==
        0) /* we have a section break */
      break;
  ...
  while (!feof(inFile)) {
    ...
    while (!feof(inFile)) {
      ...
      if (count > 0) {
         ...
         * specified here are the entrance nodes
         */
  ...
  if (systemCallSection) {
    while (!feof(inFile)) {
      ...
      if (temp >= 0) /* valid key */
      {
        ...
        for (i = 0;
             fileString->string[i] != '\0' && fileString->string[i] != ' ';
             ++i) {
        }
...
bool parseConfigFile(char *configFileName, Configuration *config) {
  ...
  if (feof(configFile) ||
      (strncmp("Pathfinder Configuration", data, 24) != 0)) {
    printf("Error in config file: doesn't start with \"Pathfinder "
           "Configuration\"\n");
    return false;
  }
