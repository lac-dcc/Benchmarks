 ...
 * a specific node and the label that is at the start of the label array
 * argument. The label array is an array of char pointers that is terminated
 * with a NULL pointer. The first pointer is the "current" label.
 *
 * The node's edge nodes are checked against the current label. If none of them
 * match, then this method recurses with each edge node and the current label.
 *
 * If an edge matches, then we are beginning a search for the next "leg" of the
 * labels. We recurse with that specific edge and the "next" label. That is the
 * [1] element of the label array. This starts the search along the next leg -
 * at this point we have to start a new "result" vector, because we check that
 * vector for loops. It MAY be the case that the next path segment passes
 * through nodes already traversed in a previous leg. A new "result" vector will
 * solve that.
 *
 * If the next element in the labels list is NULL, the matching edge signifies
 * the end of our search! VICTORY!
 *
                   ...
                   Bitfield *visited) /* NodePtrVec *visited ) */
{
   ...
   * approach would be to have unique hash values for each label, and thereby be
   ...
   * our edge nodes have a connection to a labeled node.
   */
  for (edge = node->edges; edge != NULL; edge = edge->nextEdge) {
...
static void logResult(NodeVecVec *storage, NodePtrVec *result,
  ...
  if (!options->multiThreaded &&
      options->doStatistics) /* statistics are not thread-safe yet */
    logStats(result);
 ...
 * in that it 1) doesn't stop with the first discovery of the specified path and
 * 2) records the discovered path. To do this, it doesn't create a new result
 * vector for
 */
 ...
 * associated with the labels (each label search will return a NodePtrVec*), and
 * determine if paths exist passing through the set of labels. This returns the
 * first FOUND path, not the optimal one. We could do that in a future iteration
 * of the code by keeping track of all paths found, and going with the shortest.
 *
 * The labels passed in is an array of char pointers. The array is null
 * terminated - i.e. the last valid entry in the array is followed by a pointer
 * with NULL in it.
 */
bool findLabelPath(Graph *graph, Signature labels, NodePtrVec *result,
   ...
   * to iterate through the start nodes and see if they can get to the next
   * label...
   */
  ...
  for (i = 0; i < startNodes->contentSize && !found; ++i) {
    ...
    if (!found &&
        result->contentSize !=
            0) /* If it's not found the result SHOULD be empty, however... */
...
int findAllPossibleLegs(Graph *graph, SearchType searchType) {
  ...
  //reduction(+:searches) #pragma omp single
  {
 ...
 * findAllPossibleLegs. However, the requirement that all the legs be logged
 * requires a more sophisticated means of parallelization in order to do a
...
#pragma omp collapse(2)
  ...
  lastingResults[i]->contentSize ); for ( j = 0; j <
  lastingResults[i]->contentSize; ++j )
      {
          printf("\t");
   ...
   * to buildGraphFromPaths. Until then, however, we're only going to build the
   * most minimal graph possible.
   */
