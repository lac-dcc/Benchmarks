 ...
 * currently points to a number, we iterate past that number, past any
 * whitespace or non-number and point to the first digit character we find. If
 * input currently points a non- digit character, we find the next digit
 * character and return a pointer to it. If no non-digit character is found with
 * these rules, we return null.
 */
