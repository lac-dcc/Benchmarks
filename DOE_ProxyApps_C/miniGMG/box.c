...
int create_box(box_type *box, int numGrids, int low_i, int low_j, int low_k,
  ...
  // multiple of  32 bytes (required for AVX/QPX) box->plane  =(
  // ((dim_j+2*ghosts)*box->pencil)+paddingToAvoidStencilCleanup+0x1) & ~0x1; //
  // multiple of  16 bytes (required for SSE) printf("%2d^2 = %5d ->
  ...
  // 32KB / 8way / 8bytes / 13 arrays if(dim_i>=32){while( ((box->volume %  256)
  // !=  40) )box->volume+=8;} // 16KB / 8way / 8bytes / 7 arrays
