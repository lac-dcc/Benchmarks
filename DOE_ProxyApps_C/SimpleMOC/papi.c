...
void papi_serial_init(void) {
  ...
  if ((PAPI_thread_init((long unsigned int (*)(void))pthread_self)) !=
      PAPI_OK) {
...
!= PAPI_OK){ PAPI_perror("PAPI_thread_init"); exit(1);
 }
...
#pragma omp master
  {
    ...
    if (I->papi_event_set == 7) {
      printf("%-30s %.2lf%%\n",
             "Data TLB Load Miss Rate: ", tlb_load_m / (double)tlb_load * 100);
