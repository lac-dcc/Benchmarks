...
void print_help_message(void) {
  ...
  printf("--block_change - (>= 0) number of levels a block can change in a "
         "timestep\n");
  ...
  printf("--stencil - 7 or 27 point (27 will not work with refinement (except "
         "uniform))\n");
  ...
  printf("         0 minimal sends, 1 send ghosts, 2 send ghosts and process "
         "on send\n");
  printf("--permute - altenates directions in communication\n");
  printf("--refine_ghost - use full extent of block (including ghosts) to "
...
int check_input(void) {
  ...
  if (stencil == 27 && num_refine && !uniform_refine)
    printf("WARNING: 27 point stencil with non-uniform refinement: answers may "
           "diverge\n");
