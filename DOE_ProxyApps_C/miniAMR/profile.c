  ...
  if (!my_pe) {
    ...
    if (report_perf & 2) {
      ...
      for (i = 0; i < 4; i++) {
        ...
        fprintf(fp, "                              average    stddev  minimum  "
                    "maximum\n");
    ...
    if (report_perf & 4) {
      ...
      /*
         ...
         average[74+9*i]);
               ...
               printf("---------------------------------------------\n\n");
               printf("     Time: ave, stddev, min, max (sec): %lf\n\n",
         average[39]); printf("        calc: ave, stddev, min, max (sec):
         ...
         (num_tsteps*stages_per_ts)), nb_min, nb_max); printf("     Max blocks
         on a processor at any time: %d\n", global_max_b); printf("     total
         blocks split     : %lf\n", average[104]); printf("     total blocks
         reformed  : %lf\n\n", average[105]);
